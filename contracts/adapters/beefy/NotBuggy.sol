...
contract VelodromeCalldataSourceNotBug is IBeefyCalldataLibrary {
    ...
    function _getStableSwap(
        ...
        // Calculate the fee
        uint256 fee = (halfInvestment * 5) / 10000;

        // Update reserves after the swap operation
        reserveA += halfInvestment;
        reserveB -= out;

        (uint256 amountA, uint256 amountB) = _quoteAddLiquidity(
            reserveA,
            reserveB,
            halfInvestment,
            out
        );
        console.log(amountA, halfInvestment);
